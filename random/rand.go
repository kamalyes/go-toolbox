 ...
 * @LastEditTime: 2024-08-03 22:40:28
...
import (
	...
	"github.com/kamalyes/go-toolbox/randx"
)
...
var (
	...
	newRandSend  = randx.NewRand()
	fastRandSend = randx.FastRand()
)
...
func FRandUint32(min, max uint32) uint32 {
	...
	return randx.FastRandn(max-min) + min
}
...
func FastIntn(n int) int {
	...
	if n <= math.MaxUint32 {
		return int(randx.FastRandn(uint32(n)))
	}
...
func FRandBytesLetters(n int, letters string) []byte {
	...
	b := make([]byte, n)
	for i, cache, remain := n-1, fastRandSend, LETTER_IDX_MAX; i >= 0; {
		if remain == 0 {
			cache, remain = fastRandSend, LETTER_IDX_MAX
		}
