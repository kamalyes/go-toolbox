 ...
 * @LastEditTime: 2025-06-13 15:15:26
...
func (r *Runner[T]) Run(fn func(ctx context.Context) (T, error)) (result T, err error) {
	...
	// 内部辅助函数，根据 err 是否为 nil，调用不同的回调函数
	callCallbacks := func(result T, err error) {
		if err != nil {
			// 任务执行失败，调用失败回调（如果设置了）
			if r.onError != nil {
...
// lock 必须实现 sync.Locker 接口
func (r *Runner[T]) RunWithLock(lock sync.Locker, fn func(ctx context.Context) (T, error)) (T, error) {
	if lock == nil {
		var zero T
