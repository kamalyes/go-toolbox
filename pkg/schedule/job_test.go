...
func TestConcurrentAddJobs(t *testing.T) {
	...
	for i := 0; i < jobCount; i++ {
		...
		go func(id int) {
			...
			job, exists := schedule.jobs[jobConfig.id]
			// 断言任务的各个属性是否被正确设置

			assert.True(t, exists, "Job should exist after addition")
...
func TestDelJob(t *testing.T) {
	...
	// 确认任务已添加
	assert.NotNil(t, schedule.GetJobStatus(job.id))

	// 删除任务
	schedule.DelJob(job.id)

	// 删除后任务应不存在
	assert.Nil(t, schedule.GetJobStatus(job.id))
}
...
func TestAbortJob(t *testing.T) {
	...
	// 任务状态应被标记为终止
	abortedJob := schedule.GetJobStatus(job.id)
...
func TestGetJobStatus(t *testing.T) {
	...
	// 获取已存在任务状态
	retrieved := schedule.GetJobStatus(job.id)
	assert.NotNil(t, retrieved)
	assert.Equal(t, job.id, retrieved.GetId())

	// 获取不存在任务状态应返回 nil
	nonExistent := schedule.GetJobStatus("nonexistent")
