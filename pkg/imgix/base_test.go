 ...
 * @LastEditTime: 2025-12-12 22:19:03
 * @FilePath: \go-toolbox\pkg\imgix\base_test.go
 * @Description:
 *
...
func createImgixTestImage(format ImageFormat, width, height int, color color.Color) (image.Image, *bytes.Buffer, error) {
...
func testValidPNGImage(t *testing.T) {
	_, buf, err := createImgixTestImage(PNG, 1, 1, image.White)
	assert.NoError(t, err)

	server := mockServer(buf.Bytes(), http.StatusOK) // 启动模拟服务器
	defer server.Close()                             // 确保服务器在测试结束后关闭

	img, err := LoadImageFromURL(server.URL) // 调用函数加载图片
...
func testValidJPEGImage(t *testing.T) {
	_, buf, err := createImgixTestImage(JPEG, 1, 1, image.White)
	assert.NoError(t, err)
...
func testInvalidURL(t *testing.T) {
	_, err := LoadImageFromURL("http://invalid-url") // 调用函数加载无效的 URL
...
func test404Error(t *testing.T) {
	...
	_, err := LoadImageFromURL(server.URL) // 调用函数加载图片
	assert.Error(t, err)                   // 期望返回错误
}
...
func testNetworkError(t *testing.T) {
	_, err := LoadImageFromURL("http://127.0.0.1:9999") // 假设该端口没有服务在运行
	assert.Error(t, err)                                // 期望返回错误
}

func TestWriterImage(t *testing.T) {
	// 创建一个测试用的图像
	img, _, err := createImgixTestImage(JPEG, 100, 100, color.RGBA{255, 0, 0, 255})
	assert.NoError(t, err)

	// 测试 JPEG
	var buf bytes.Buffer
	err = WriterImage(img, 80, JPEG, &buf)
	assert.NoError(t, err)          // 期望没有错误
	assert.Greater(t, buf.Len(), 0) // 期望输出不为空

	// 测试 PNG
	buf.Reset() // 清空缓冲区
	img, _, err = createImgixTestImage(PNG, 100, 100, color.RGBA{255, 0, 0, 255})
	assert.NoError(t, err)
	err = WriterImage(img, 0, PNG, &buf)
	assert.NoError(t, err)          // 期望没有错误
	assert.Greater(t, buf.Len(), 0) // 期望输出不为空

	// 测试 GIF
	buf.Reset() // 清空缓冲区
	img, _, err = createImgixTestImage(GIF, 100, 100, color.RGBA{255, 0, 0, 255})
	assert.NoError(t, err)
	err = WriterImage(img, 0, GIF, &buf)
	assert.NoError(t, err)          // 期望没有错误
	assert.Greater(t, buf.Len(), 0) // 期望输出不为空
}
...
func TestWriterImageFromBytes(t *testing.T) {
	// 创建一个测试用的 JPEG 图像
	_, jpegImageBuf, err := createImgixTestImage(JPEG, 1, 1, image.White)
	assert.NoError(t, err)

	// 测试 WriterImageFromBytes 函数
	var buf bytes.Buffer
	err = WriterImageFromBytes(jpegImageBuf.Bytes(), 80, PNG, &buf)
	assert.NoError(t, err)          // 期望没有错误
	assert.Greater(t, buf.Len(), 0) // 期望输出不为空

	// 测试从无效的字节切片加载图像
	err = WriterImageFromBytes([]byte("not an image"), 80, PNG, &buf)
	assert.Error(t, err) // 期望返回错误
}

func TestSaveBufToImageFile(t *testing.T) {
	formats := []ImageFormat{JPEG, PNG, GIF}
	...
	for _, format := range formats {
		t.Run(format.String(), func(t *testing.T) {
			...
			err = SaveBufToImageFile(buf, filePath, format)
			assert.NoError(t, err) // 期望没有错误
	...
	err := SaveBufToImageFile(invalidBuf, "test_invalid_image.png", PNG)
	assert.Error(t, err) // 期望返回错误

	_, buf, err := createImgixTestImage(JPEG, 100, 100, image.White)

	// 测试使用不支持的格式
	err = SaveBufToImageFile(buf, "test_image.invalid", ImageFormat(100))
	...
	err = SaveBufToImageFile(buf, "readonly_dir/test_image.png", PNG)
	assert.Error(t, err) // 期望返回错误
...
func TestLoadImageFromFile(t *testing.T) {
	// 创建一个测试用的 JPEG 图像文件
	_, buf, err := createImgixTestImage(JPEG, 10, 10, color.RGBA{255, 0, 0, 255})
	assert.NoError(t, err)
	...
	// 测试加载有效的图像文件
	loadedImg, err := LoadImageFromFile(filePath)
	assert.NoError(t, err)      // 期望没有错误
...
func TestLoadImageFromFileInvalidPath(t *testing.T) {
	_, err := LoadImageFromFile("invalid/path/to/image.jpg") // 调用函数加载无效路径
	assert.Error(t, err)                                     // 期望返回错误
}
...
func TestLoadImageFromFileUnsupportedFormat(t *testing.T) {
	...
	// 测试加载无效的图像文件
	_, err = LoadImageFromFile(tempFile.Name())
	assert.Error(t, err) // 期望返回错误
}
...
func TestEncodeImageToBase64(t *testing.T) {
	...
	// 测试有效的图像编码为 Base64
	base64Str, err := EncodeImageToBase64(img, JPEG, 80)
	assert.NoError(t, err)                                   // 期望没有错误
...
func TestLoadImageFromFileAndEncodeToBase64(t *testing.T) {
	...
	base64Str, err := LoadImageFromFileAndEncodeToBase64(filePath, JPEG, 80)
	assert.NoError(t, err)                                   // 期望没有错误
	assert.NotEmpty(t, base64Str)                            // 期望返回的字符串不为空
	assert.Contains(t, base64Str, "data:image/jpeg;base64,") // 确保包含正确的前缀
}
...
func TestLoadImageFromFileAndEncodeToBase64InvalidPath(t *testing.T) {
	// 测试加载无效文件路径
	base64Str, err := LoadImageFromFileAndEncodeToBase64("invalid/path/to/image.jpg", JPEG, 80)
	assert.Error(t, err)       // 期望返回错误
...
func TestLoadImageFromFileAndEncodeToBase64UnsupportedFormat(t *testing.T) {
	...
	base64Str, err := LoadImageFromFileAndEncodeToBase64(tempFile.Name(), JPEG, 80)
	assert.Error(t, err)       // 期望返回错误
	assert.Empty(t, base64Str) // 期望返回的字符串为空
}
...
func TestAddOverlay(t *testing.T) {
	...
	// 调用 AddOverlay 函数
	maskedImg := AddOverlay(testImg, overlayColor)
...
func TestMatchesAnyPattern(t *testing.T) {
	...
	for _, c := range cases {
		got := MatchesAnyPattern(c.input, c.patterns)
		if got != c.want {
