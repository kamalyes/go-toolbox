 ...
 * @LastEditTime: 2024-12-13 10:50:15
...
type GraphicsRenderer struct {
	GgCtx      *gg.Context
	dashLength float64      // 虚线段长度
	gapLength  float64      // 虚线间隔长度
	readMu     sync.RWMutex // 读锁
...
func NewGraphicsRenderer(ctx *gg.Context) *GraphicsRenderer {
	return &GraphicsRenderer{GgCtx: ctx}
}
...
func (g *GraphicsRenderer) UseDefaultDashed() {
	g.drawWithStroke(func() {
		g.GgCtx.SetDash(g.dashLength, g.gapLength, 0.0)
	}, false)
}
...
func (g *GraphicsRenderer) SetDashStyle(dashLength, gapLength float64) {
	g.drawWithStroke(func() {
		g.dashLength = dashLength
		g.gapLength = gapLength
	}, false)
}
