 ...
 * @FilePath: \go-toolbox\pkg\mathx\entropy_test.go
 * @Description:
 *
...
func TestCalcEntropy(t *testing.T) {
	...
	assert.InDelta(expectedEntropy1, CalcEntropy(data1), 1e-9)
	...
	assert.InDelta(expectedEntropy2, CalcEntropy(data2), 1e-9)

	// 测试用例3：空分布
	data3 := map[interface{}]int{}
	expectedEntropy3 := 0.0
	assert.Equal(expectedEntropy3, CalcEntropy(data3))
	...
	assert.Equal(expectedEntropy4, CalcEntropy(data4))
}
