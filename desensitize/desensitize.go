 ...
 * @LastEditTime: 2024-08-03 10:00:59
...
// 手机号脱敏
func phoneNumber(str string, start, end int) string {
	// 空判断
	if validator.IsEmptyValue(reflect.ValueOf(str)) {
		return str
	}
...
// 车牌号脱敏
func carLicense(str string) string {
	// 空判断
	if validator.IsEmptyValue(reflect.ValueOf(str)) {
		return str
	}
...
// 银行卡号脱敏
func bankCard(str string, cardLength int) string {
	// 如果卡号为空，则直接返回原卡号
	if validator.IsEmptyValue(reflect.ValueOf(str)) {
		return str
	}
