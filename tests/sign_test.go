 ...
 * @FilePath: \go-toolbox\tests\sign_test.go
 * @Description:
 *
...
func TestHmacSha256Base64(t *testing.T) {
	...
	signature := sign.HmacSha256Base64(message, secret)
...
func TestHmacSha256Hex(t *testing.T) {
	...
	signature := sign.HmacSha256Hex(message, secret)
	if signature != expected {
		t.Errorf("HmacSha256Hex: Expected: %s, but got: %s", expected, signature)
	}

	// Test with empty secret
	signature = sign.HmacSha256Hex(message, "")
