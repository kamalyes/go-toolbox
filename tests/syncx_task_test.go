 ...
 * @LastEditTime: 2025-07-23 13:15:55
...
func TestNewTaskManager(t *testing.T) {
	...
	assert.Equal(t, 2, tm.GetConcurrency(), "确保并发数设置正确") // 确保并发数设置正确
}
...
func TestRunTask(t *testing.T) {
	...
	task := syncx.NewTask[string, string, string]("task1", func(ctx context.Context, input string) (string, error) {
		fmt.Println("Task function is executing") // 添加日志
		return input, nil
	}, "hello")
	...
	time.Sleep(20 * time.Millisecond) // 模拟任务执行时间
	// 验证任务状态和结果
	assert.Equal(t, syncx.Completed, task.GetState(), "任务状态应为已完成")
	result := task.GetResult()
	assert.Equal(t, "hello", result, "任务结果应为 'hello'")
}
...
func TestCancelAllTasks(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		task := syncx.NewTask[string, string, string](fmt.Sprintf("task%d", i), func(ctx context.Context, input string) (string, error) {
			time.Sleep(20 * time.Millisecond) // 模拟长时间运行的任务
			return input, nil
		}, fmt.Sprintf("hello %d", i))
		tm.AddTask(task)
	}
	tm.CancelAll()                    // 取消所有任务
	time.Sleep(10 * time.Millisecond) // 模拟长时间运行的任务
	go tm.Run()                       // 执行所有任务
...
func TestTaskHistory(t *testing.T) {
	...
	history := tm.GetTaskHistory("task1")
	assert.NotNil(t, history, "任务历史记录应存在")
	assert.Equal(t, 1, len(history), "任务历史记录应包含一条记录")
	assert.Equal(t, syncx.Completed, history[0].State, "历史记录状态应为已完成")
	assert.Equal(t, "hello", history[0].Result, "历史记录结果应为 'hello'")
}
...
func TestTaskMultipleExecutions(t *testing.T) {
	...
	tm.AddTask(task) // 添加任务
	tm.Run()         // 第一次执行
	assert.Equal(t, syncx.Completed, task.GetState(), "第一次执行后任务状态应为已完成")
	tm.Release() // 释放资源

	// 再次添加同一任务并执行
	tm.AddTask(task) // 重复添加任务
	tm.Run()         // 第二次执行
	assert.Equal(t, syncx.Completed, task.GetState(), "第二次执行后任务状态应为已完成")
}

// 测试任务历史记录的多次执行
func TestTaskHistoryMultipleExecutions(t *testing.T) {
	tm := syncx.NewTaskManager[string, string, string](2)
	task := syncx.NewTask[string, string, string]("task1", func(ctx context.Context, input string) (string, error) {
		return input, nil
	}, "hello")
	...
	history1 := tm.GetTaskHistory("task1")
	assert.Equal(t, 1, len(history1), "历史记录应包含一条记录")
	tm.Release() // 释放资源

	// 重新创建任务实例，确保是不同的任务
	task2 := syncx.NewTask[string, string, string]("task1", func(ctx context.Context, input string) (string, error) {
		return input, nil
	}, "hello")

	tm.AddTask(task2) // 再次添加相同任务
	tm.Run()          // 第二次执行

	history2 := tm.GetTaskHistory("task1")
	assert.Equal(t, 2, len(history2), "历史记录应包含两条记录")
}
...
func TestTaskManagerCancelRunningTasks(t *testing.T) {
	...
	time.Sleep(100 * time.Millisecond) // 模拟较长的任务执行时间

	// 验证任务状态
	assert.Equal(t, syncx.Cancelled, task1.GetState(), "任务1状态应为已取消")
	assert.Equal(t, syncx.Cancelled, task2.GetState(), "任务2状态应为已取消")
	tm.Release() // 释放资源

	go tm.Run()                        // 执行任务
	time.Sleep(100 * time.Millisecond) // 模拟较长的任务执行时间

	assert.Equal(t, syncx.Completed, task1.GetState(), "任务1状态应为已完成")
	assert.Equal(t, syncx.Completed, task2.GetState(), "任务2状态应为已完成")
}

// 测试任务历史记录为空的情况
func TestTaskManagerEmptyHistory(t *testing.T) {
	tm := syncx.NewTaskManager[string, string, string](2)

	history := tm.GetTaskHistory("nonexistent")
	assert.Nil(t, history, "不存在的任务应没有历史记录")
}
...
func TestTaskHistoryLimit(t *testing.T) {
	tm := syncx.NewTaskManager[string, string, string](2)
	tm.SetMaxHistorySize(2) // 设置最大历史记录为2
	...
	history := tm.GetTaskHistory("task0")
	assert.Equal(t, 2, len(history), "历史记录应只包含最新的两条记录")
}
...
func TestTaskRetriesExceedMax(t *testing.T) {
	...
	task.SetMaxRetries(3).SetRetryInterval(10 * time.Millisecond)

	tm.AddTask(task)
	tm.Run()

	assert.Equal(t, syncx.Failed, task.GetState(), "任务状态应为失败")
	assert.Equal(t, 3, task.GetRetryCount(), "任务重试次数应为 3")
}
...
func TestTaskRetriesDuringCancellation(t *testing.T) {
	...
	task := syncx.NewTask[string, string, string]("task1", func(ctx context.Context, input string) (string, error) {
		time.Sleep(50 * time.Millisecond) // 模拟长时间执行
		return "", errors.New("task failed")
	}, "input")

	task.SetMaxRetries(5).SetRetryInterval(10 * time.Millisecond)

	tm.AddTask(task)

	go tm.Run()
	time.Sleep(20 * time.Millisecond) // 等待一段时间
	tm.Cancel("task1")                // 取消任务
...
func TestTaskExecutionOrder(t *testing.T) {
	...
	assert.Equal(t, expectedOrder, executionOrder, "任务执行顺序应为 task1, task2")
}
...
func TestTaskManager_TrunDown(t *testing.T) {
	...
	taskManager.SetTrunDown(func() (string, error) {
		time.Sleep(1 * time.Second) // 模拟关闭过程
