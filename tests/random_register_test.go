...
func TestRegisterBasicGenerator(t *testing.T) {
	...
	// 注册一些自定义生成器
	random.RegisterGenerator("company", func() interface{} {
		companies := []string{"Google", "Microsoft", "Apple", "Amazon", "Meta"}
		return companies[random.FRandInt(0, len(companies)-1)]
	})

	random.RegisterGenerator("job_title", func() interface{} {
		titles := []string{"软件工程师", "产品经理", "架构师", "技术总监", "CTO"}
		return titles[random.FRandInt(0, len(titles)-1)]
	})

	// 验证注册成功
	assert.Contains(t, random.ListRegisteredGenerators(), "company")
	assert.Contains(t, random.ListRegisteredGenerators(), "job_title")
	assert.Equal(t, 2, len(random.ListRegisteredGenerators()))

	// 测试生成器是否存在
	companyGen, exists := random.GetGenerator("company")
...
func TestRegisterNumberGenerator(t *testing.T) {
	...
	// 注册用户ID生成器（整数）
	random.RegisterGenerator("user_id", func() interface{} {
		return random.FRandInt(10000, 99999)
	})

	// 注册薪资范围生成器
	random.RegisterGenerator("salary_range", func() interface{} {
		// 生成10k到100k的薪资
		return random.FRandInt(10000, 100000)
	})

	// 注册绩效分数生成器（浮点数）
	random.RegisterGenerator("performance_score", func() interface{} {
		return float64(random.FRandInt(60, 100)) + random.RandFloat(0, 1)
	})

	// 验证注册成功
	generators := random.ListRegisteredGenerators()
	...
	// 测试用户ID生成器
	userIDGen, _ := random.GetGenerator("user_id")
	userID := userIDGen()
	...
	// 测试薪资生成器
	salaryGen, _ := random.GetGenerator("salary_range")
	salary := salaryGen()
	...
	// 测试绩效分数生成器
	scoreGen, _ := random.GetGenerator("performance_score")
	score := scoreGen()
...
func TestRegisterBooleanGenerator(t *testing.T) {
	...
	random.RegisterGenerator("vip_status", func() interface{} {
		return random.FRandInt(1, 100) <= 20 // 20%概率为true
	})

	vipGen, _ := random.GetGenerator("vip_status")
...
func TestRegisterComplexGenerator(t *testing.T) {
	...
	// 注册全名生成器
	random.RegisterGenerator("full_name", func() interface{} {
		firstNames := []string{"张", "李", "王", "刘", "陈", "杨", "黄", "赵", "吴", "周"}
		lastNames := []string{"伟", "芳", "娜", "秀英", "敏", "静", "丽", "强", "磊", "军"}
		return firstNames[random.FRandInt(0, len(firstNames)-1)] + lastNames[random.FRandInt(0, len(lastNames)-1)]
	})

	// 注册企业邮箱生成器
	random.RegisterGenerator("business_email", func() interface{} {
		domains := []string{"company.com", "corp.com", "tech.com", "enterprise.com"}
		username := random.FRandString(6)
		domain := domains[random.FRandInt(0, len(domains)-1)]
		return strings.ToLower(username) + "@" + domain
	})

	// 测试全名生成器
	nameGen, _ := random.GetGenerator("full_name")
	name := nameGen().(string)
	...
	// 测试企业邮箱生成器
	emailGen, _ := random.GetGenerator("business_email")
...
func TestGenerateWithRegisteredGenerators(t *testing.T) {
	...
	random.RegisterGenerator("full_name", func() interface{} {
		...
		return firstNames[random.FRandInt(0, len(firstNames)-1)]
	})
	...
	random.RegisterGenerator("salary_range", func() interface{} {
		return random.FRandInt(8000, 50000)
	})

	random.RegisterGenerator("business_email", func() interface{} {
		domains := []string{"company.com", "tech.cn", "corp.com"}
		return random.FRandString(6) + "@" + domains[random.FRandInt(0, len(domains)-1)]
	})

	random.RegisterGenerator("vip_status", func() interface{} {
		return random.FRandInt(1, 100) <= 30 // 30%概率为VIP
	})

	random.RegisterGenerator("performance_score", func() interface{} {
		return float64(random.FRandInt(70, 95)) + random.RandFloat(0, 1)
	})

	// 生成测试用户
	var user TestUser
	_, _, err := random.GenerateRandModel(&user)
...
func TestUnregisterGenerator(t *testing.T) {
	...
	// 注册几个生成器
	random.RegisterGenerator("test1", func() interface{} { return "value1" })
	random.RegisterGenerator("test2", func() interface{} { return "value2" })
	random.RegisterGenerator("test3", func() interface{} { return "value3" })

	assert.Equal(t, 3, len(random.ListRegisteredGenerators()))

	// 注销一个生成器
	random.UnregisterGenerator("test2")
	generators := random.ListRegisteredGenerators()
	...
	// 验证注销的生成器不存在
	_, exists := random.GetGenerator("test2")
	assert.False(t, exists)

	// 验证其他生成器仍然存在
	_, exists = random.GetGenerator("test1")
	assert.True(t, exists)
}
...
func TestClearAllGenerators(t *testing.T) {
	...
	// 清空所有生成器
	random.ClearAllGenerators()
	assert.Equal(t, 0, len(random.ListRegisteredGenerators()))

	// 验证所有生成器都被清除
	_, exists1 := random.GetGenerator("test1")
	_, exists2 := random.GetGenerator("test2")
	_, exists3 := random.GetGenerator("test3")
...
func TestRegisterGeneratorOverride(t *testing.T) {
	...
	// 注册第一个生成器
	random.RegisterGenerator("test", func() interface{} { return "old_value" })
	
	gen1, _ := random.GetGenerator("test")
	assert.Equal(t, "old_value", gen1())

	// 注册同名生成器进行覆盖
	random.RegisterGenerator("test", func() interface{} { return "new_value" })
	
	gen2, _ := random.GetGenerator("test")
	assert.Equal(t, "new_value", gen2())

	// 验证只有一个生成器
	assert.Equal(t, 1, len(random.ListRegisteredGenerators()))
}
...
func TestRegisterNilGenerator(t *testing.T) {
	...
	// 注册nil生成器应该不会崩溃
	random.RegisterGenerator("nil_test", nil)
	
	// 验证nil生成器不会被注册
	_, exists := random.GetGenerator("nil_test")
	assert.False(t, exists)
	assert.Equal(t, 0, len(random.ListRegisteredGenerators()))
}
...
func TestGeneratorWithTypeConversion(t *testing.T) {
	...
	// 注册返回不同类型的生成器
	random.RegisterGenerator("int_to_string", func() interface{} {
		return 12345 // 返回int，但目标字段是string
	})

	random.RegisterGenerator("string_to_int", func() interface{} {
		return "67890" // 返回string，但目标字段是int
	})

	random.RegisterGenerator("int_to_float", func() interface{} {
		return 123 // 返回int，但目标字段是float64
	})

	random.RegisterGenerator("int_to_bool", func() interface{} {
		return 1 // 返回int，但目标字段是bool（非零值应转换为true）
	})

	// 生成测试对象
	var test TestConversion
	_, _, err := random.GenerateRandModel(&test)
	assert.NoError(t, err)
...
func TestGeneratorPriority(t *testing.T) {
	...
	random.RegisterGenerator("email", func() interface{} {
