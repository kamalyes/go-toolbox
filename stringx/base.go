...
import (
	...
	"github.com/kamalyes/go-toolbox/numberx"
)
...
// 返回的数值越大，表示相似度越低
func levenshteinDistance(str1 string, str2 string) (int, error) {
	...
	return dist[len1][len2], nil
}

// Similar 计算两个字符串的相似度
// 只比较两个字符串字母、数字、汉字部分，其他符号去除
// 计算出两个字符串最大子串，除以最长的字符串，结果即为相似度
func Similar(str1 string, str2 string) (float64, error) {
	distance, err := levenshteinDistance(filterSymbols(str1), filterSymbols(str2))
	if err != nil {
		return 0.0, err
	}
...
func SimilarScale(str1 string, str2 string, scale int) (string, error) {
	distance, err := levenshteinDistance(filterSymbols(str1), filterSymbols(str2))
	if err != nil {
		return "", err
	}
	...
	return fmt.Sprintf("%.*f%%", scale, similarity), nil
}
